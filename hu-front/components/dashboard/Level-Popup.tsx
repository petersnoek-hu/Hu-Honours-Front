...
export default function Popup({ level, titel, onPress }: PopUpProps) {
  return (
    <View className="items-center bottom-2 z-50">
      <View className="bg-[#F15A33] rounded-xl px-6 py-4 w-full">
        <Text className="font-bold color-[#f6f6f6]">
          Level {level}: {titel}
        </Text>
        <Pressable
          className="bg-[#f6f6f6] rounded-lg justify-center items-center mt-4"
          onPress={onPress}
        >
          <Text className="font-bold pt-2 pb-2 color-[#EE5B39]">
            Start Level
          </Text>
        </Pressable>
      </View>
...
export default function Popup({level, titel, handlePress}:PopUpProps){
    return(
        <View className="items-center bottom-2 z-50">
            <View className="bg-[#F15A33] rounded-xl px-6 py-4 w-full">
